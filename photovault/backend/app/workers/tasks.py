...
@celery_app.task(bind=True, name="reindex_local_media")
def reindex_local_media(self, job_id: str, user_id: Optional[str] = None):
    """Reindex local uploaded media — scan /media/{user_id}/uploads/ and fix missing DB records / thumbnails."""
    from app.models.models import ProcessingJob, User, Media
    from app.services.media.processor import (
        generate_all_thumbnails, get_file_hash, get_mime_type,
    ...
    try:
        ...
        # Determine which users to reindex
        if user_id:
            users = db.query(User).filter(User.id == UUID(user_id), User.is_active == True).all()
        else:
        ...
        for user in users:
            uploads_dir = os.path.join(settings.LOCAL_STORAGE_PATH, str(user.id), "uploads")
            ...
            for fname in os.listdir(uploads_dir):
                ...
                try:
                    ...
                    if existing:
                        ...
                        if not existing.thumbnail_small or not os.path.exists(existing.thumbnail_small or ""):
                            thumbs = generate_all_thumbnails(file_path, str(existing.id), existing.media_type)
                            existing.thumbnail_small = thumbs.get("thumbnail_small")
                            existing.thumbnail_medium = thumbs.get("thumbnail_medium")
                            existing.thumbnail_large = thumbs.get("thumbnail_large")
                            db.commit()
                        processed += 1
                        continue
                    ...
                    media = Media(
                        ...
                        filename=fname,
                        original_path=file_path,
                        relative_path=os.path.join(str(user.id), "uploads", fname),
                        file_hash=file_hash,
                    ...
                    thumbs = generate_all_thumbnails(file_path, str(media.id), media_type)
                    media.thumbnail_small = thumbs.get("thumbnail_small")
                    media.thumbnail_medium = thumbs.get("thumbnail_medium")
                    media.thumbnail_large = thumbs.get("thumbnail_large")
                    db.commit()
                    processed += 1
                ...
                job.processed_items = processed
                db.commit()
                self.update_state(state="PROGRESS", meta={"processed": processed, "total": total_files})
        ...
        job.completed_at = datetime.utcnow()
        db.commit()
        logger.info(f"Reindex completed: {processed} processed, {failed} failed")
...
def cleanup_stale_jobs():
    """Mark jobs that have been stuck in 'pending' or 'running' for > 2 hours as failed."""
    ...
    try:
        ...
        for job in stale:
            job.status = "failed"
            job.error_message = "Job timed out — worker may have restarted while this job was queued."
            job.completed_at = datetime.utcnow()
        if stale:
            db.commit()
