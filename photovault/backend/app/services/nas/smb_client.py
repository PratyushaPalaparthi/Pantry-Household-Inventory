...
def mount_nas_share(
    ...
    try:
        ...
        # Mount using CIFS without invoking a shell, and without putting passwords
        # on the process command line (use a temp credentials file instead).
        import subprocess

        creds_path = None
        try:
            with tempfile.NamedTemporaryFile("w", prefix="photovault-cifs-", delete=False) as f:
                creds_path = f.name
                f.write(f"username={username or ''}\n")
                f.write(f"password={password or ''}\n")
            os.chmod(creds_path, 0o600)

            options = f"credentials={creds_path},vers=3.0"
            cmd = ["mount", "-t", "cifs", f"//{host}/{share_name}", mount_point, "-o", options]
            ...
            if creds_path:
                try:
                    ...
                    # Best-effort cleanup; avoid raising during mount error handling.
                    pass
