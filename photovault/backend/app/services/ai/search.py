...
def get_text_embedding_safe(query: str):
    """Best-effort CLIP embedding lookup with graceful fallback."""
    try:
        from app.services.ai.clip_service import get_text_embedding
    except Exception as exc:
        logger.warning("CLIP import failed, falling back to non-vector search", error=str(exc))
        return None

    try:
        return get_text_embedding(query)
    except Exception as exc:
        logger.warning("CLIP embedding failed, falling back to non-vector search", error=str(exc))
        return None
...
async def semantic_search(
    ...
    query_embedding = get_text_embedding_safe(query)
