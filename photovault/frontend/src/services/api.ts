...
// Auth endpoints that intentionally return 401/403 — never intercept these
const AUTH_ENDPOINTS = ['/auth/login', '/auth/register', '/auth/refresh', '/auth/reset-password'];
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // Skip token-refresh logic for auth endpoints — let the caller handle errors
...
export const mediaApi = {
  ...
  getThumbnailUrl: (mediaId: string, size: 'small' | 'medium' | 'large' = 'medium') => {
    ...
    return `/api/media/${mediaId}/thumbnail/${size}${qs}`;
  },

  getFileUrl: (mediaId: string) => {
    const token = useAuthStore.getState().accessToken;
    const qs = token ? `?token=${encodeURIComponent(token)}` : '';
    return `/api/media/${mediaId}/file${qs}`;
  },
};
